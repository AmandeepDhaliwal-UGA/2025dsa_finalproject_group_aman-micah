...
final_specx <- boost_tree(#Model type is XGBoost
...
final_fitx %>%
  collect_predictions() %>%
  ggplot(aes(x = yield_adj_mg_ha, y = .pred)) +
...
ggsave(filename = file.path("../output", "xgb_pred_vs_obs.png"), 
       width = 8, 
       height = 6, 
       dpi = 300)
```
...
ggsave(filename = file.path("../output", "var_importance_xgboost.png"), 
       width = 8, 
       height = 6, 
       dpi = 300)
