...
```{r fiber-env-data}
weather <- read_csv("../data/processed/train-final-soils-weather.csv")
weather
```
```{r}
summary(weather)
...
```{r final_spec}
final_spec <- rand_forest(trees = 5,
                          mtry = 177) %>%
  # Specify the engine
set_engine("ranger",
