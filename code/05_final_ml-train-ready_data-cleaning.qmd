...
```{r rerun}
# STEP 1: Keep all weather variables
weather_cleaned <- weather_gdd_final %>%
  dplyr::select(
    site, year, hybrid, yday, 
    gdd_dbl_sine, prcp_mm_day,
    srad_w_m_2, tmax_deg_c, tmin_deg_c, vp_pa, swe_kg_m_2, dayl_s,
    harvest_doy, plant_doy
  )
...
monthly_summary <- weather_cleaned %>%
  ...
  summarise(
    ...
    max_snow = max(swe_kg_m_2, na.rm = TRUE),
    mean_daylength = mean(dayl_s, na.rm = TRUE),
    .groups = "drop"
  )
...
monthly_summary_wide <- monthly_summary %>%
  pivot_wider(
    names_from = month,
    values_from = c(
      monthly_gdd, monthly_precip,
      mean_tmax, mean_tmin,
      mean_srad, mean_vapor_pressure,
      max_snow, mean_daylength
    ),
